    ...
    var body: some View {
        if toText == nil {
            ...
            TranslatedTextField(
                ...
                onTranlateEvent: onTranslateEvent)
            .padding()
            .gradientSurface()
            .cornerRadius(15)
            .animation(.easeInOut, value: isTranslating)
...
private extension TranslateTextField {
    ...
    struct TranslatedTextField: View {
        ...
        var fromLanguage: UiLanguage
        var toLanguage: UiLanguage
        var onTranlateEvent: (TranslateEvent) -> Void
        ...
        var body: some View {
            VStack(alignment: .leading) {
                ...
                Text(fromText).foregroundColor(Color.onSurface)
                HStack {
                    Spacer()
                    Button(action: {
                    ...
                    Button(action: {
                        onTranlateEvent(TranslateEvent.CloseTranslation())
                    }){
                ...
                Text(toText).foregroundColor(Color.onSurface)
                HStack {
                    Spacer()
                    Button(action: {
