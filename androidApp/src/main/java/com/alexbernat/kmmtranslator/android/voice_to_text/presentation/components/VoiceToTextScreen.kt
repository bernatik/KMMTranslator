...
package com.alexbernat.kmmtranslator.android.voice_to_text.presentation.components
...
import com.alexbernat.kmmtranslator.android.core.theme.LightBlue
import com.alexbernat.kmmtranslator.voice_to_text.presentation.DisplayState
import com.alexbernat.kmmtranslator.voice_to_text.presentation.VoiceToTextEvent
import com.alexbernat.kmmtranslator.voice_to_text.presentation.VoiceToTextState
        ...
        onResult = { granted ->
            onEvent(
                VoiceToTextEvent.PermissionResult(
                    isGranted = granted,
                    isPermanentlyDeclined = !granted && !(context as ComponentActivity).shouldShowRequestPermissionRationale(
                ...
                AnimatedContent(targetState = state.displayState) { displayState ->
                    when (displayState) {
                        DisplayState.WAITING_TO_TALK -> Text(
                            text = stringResource(id = R.string.talking),
                            style = MaterialTheme.typography.h2,
                            textAlign = TextAlign.Center
                        )
                        DisplayState.SPEAKING -> VoiceRecorderDisplay(
                            powerRatios = state.powerRatios, modifier = Modifier
                                .fillMaxWidth()
                                .height(100.dp)
                        )
